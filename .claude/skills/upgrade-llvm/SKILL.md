...
7. Re-audit the clang-tidy check classification that `clice lint` prunes by: `python3 .claude/skills/upgrade-llvm/tidy_audit.py` (against `../llvm-project` at the new tag) lists every check with its traversal kind and the signals of TU-wide state — end-of-TU accumulation, preprocessor callbacks, container membership. New or changed checks whose findings depend on more than their own node (a whole-TU accumulator, PP-state read at match time, ancestry through a container, cross-function analysis) go into `src/compile/tidy_tu_checks.inc` with the reason; the design record is `temp/objectives/cli/lint-dedup-design.md` §四. `unit_tests --test-filter=ClangTidy` and `clice lint --verify` on the repo must stay green
8. Expect the Debug leg to catch link-list gaps: the Linux and macOS Debug packages are shared-library builds, so every library clice uses directly must be listed in `cmake/llvm.cmake` — a static link resolves symbols from any archive pulled in transitively and hides a missing entry. To check before Step 5, build `Debug` against the Step 1 `debug-asan` artifact for your platform the same way Step 2 uses `releasedbg`
