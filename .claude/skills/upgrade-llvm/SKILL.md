...
- **mingw leg**: `x86_64-w64-mingw32` is cross-built on the Linux runner in the `cross-windows-mingw` pixi environment (conda-forge's mingw-w64 sysroot and libgcc, the pixi clang); its `--configure-only` validation is `pixi run -e cross-windows-mingw python3 scripts/build-llvm.py ... --target-triple x86_64-w64-mingw32`. The package keeps `x86_64-pc-windows-msvc` as clang's default target: what Windows users compile targets MSVC unless their command says otherwise. Every pitfall of that leg is in the toolchain changelog's mingw section.
