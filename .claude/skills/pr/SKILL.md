...
- A branch stacked on a predecessor that has since been squash-merged is rebased with `git rebase --onto origin/main <predecessor-tip>` before anything else — a plain `git rebase origin/main` replays the predecessor's commits and conflicts on each of them. Never start a build or a suite on a tree with unresolved conflicts.
...
- CI failure: reproduce and fix locally, verify, then push. Digest CI logs via a subagent; don't pull raw logs into the main conversation. Infrastructure failures — a runner that lost communication, a step that timed out on `npm install` — are not yours to fix: wait for the run to finish, then `gh run rerun --failed`.
