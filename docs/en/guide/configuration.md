...
Whether `clice lint` checks matching files. `false` keeps them out: their declarations are neither checked nor reported, and a translation unit they head is not parsed. Files outside the workspace are never checked. Any matching rule saying `false` wins.
