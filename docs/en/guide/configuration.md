...
Directory for the unified on-disk cache (PCH, PCM and index artifacts). Empty defaults to `${workspace}/.clice`. The directory keeps itself out of version control and backups (a `*` .gitignore plus a CACHEDIR.TAG marker); the resolved path is printed at startup.
