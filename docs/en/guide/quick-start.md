...
By default, clice loads every `compile_commands.json` in your workspace root and its immediate subdirectories (e.g. `build/`), and the ones above a file when you open it — a nested project's database loads the first time one of its files is opened. You can name the databases explicitly with the `compile_commands` option in [clice.toml](./configuration.md), and describe files that have no database entry with a rule's `default_command`.
