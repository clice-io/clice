...
- **Unified on-disk cache.** PCH, PCM, and index artifacts live in one content-addressed store with crash-safe writes and size-bounded eviction, shared across sessions and restarts.
...
- **Toolchain baseline.** clice now builds against LLVM/Clang 21.1.8.
