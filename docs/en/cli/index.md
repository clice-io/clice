...
- `clice index --show-file <path>` prints a file's persisted rows: the content generation the rows were built from, every variant with the translation units that contributed it, and the row counts by relation kind.
