...
默认情况下，clice 会加载工作区根目录及其一级子目录（例如 `build/`）中的每一份 `compile_commands.json`，并在你打开某个文件时加载其上层各级目录中的那些——嵌套项目的数据库，会在它的某个文件第一次被打开时加载。你可以在 [clice.toml](./configuration.md) 中用 `compile_commands` 选项显式列出这些数据库，再用规则的 `default_command` 描述没有数据库条目的文件。
