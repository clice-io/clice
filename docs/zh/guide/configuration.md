...
编辑器打开的每个工作区文件夹都是一个独立的项目，各有自己的 `clice.toml`、编译数据库和缓存目录。在所有文件夹之外打开的文件，归属于其上方最近的、含有 `clice.toml` 或 `compile_commands.json`（直接位于该目录中，或位于其 `build/` 目录中）的目录；随后该目录会像已打开的文件夹一样得到服务，除非某个正在服务的项目已经加载了那份数据库。文件由数据库中列有它的项目负责编译；没有自身条目的头文件，会从源文件包含了它的项目中借用宿主，并优先选择文件夹中含有该头文件的项目。工作区符号搜索覆盖所有项目，查找引用和跳转到定义则会延伸到那些为声明该符号的文件建立了索引的项目。
