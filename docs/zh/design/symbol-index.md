...
- **符号表的局部性**。目前所有符号（包括函数内的局部变量）都被合并到 `ProjectIndex` 的全局符号表中。这导致大量只在单个文件内部有意义的符号被全局存储，增加了合并时的哈希表插入开销和内存占用。改进方向是引入多级符号表——局部符号只保留在 `MergedIndex` 分片中，`ProjectIndex` 只存储可能被跨文件引用的符号。例如：

  ```cpp
  void process() {
      auto result = compute();   // result 是局部变量，不需要进入全局符号表
      GlobalRegistry::add(result);
  }
  ```

  `result` 不会被其他文件引用，将它存入全局符号表没有意义。只有 `GlobalRegistry`、`add`、`compute` 这样可能被跨文件引用的符号才需要全局存储。
