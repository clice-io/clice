...
当工作进程持有的文档数量超过限制时，通过 LRU 策略淘汰最久未使用的文档，释放 AST 占用的内存。淘汰时工作进程向主进程发送通知，主进程清除路由表中的绑定关系。后续对该文档的请求会触发重新分配和重新编译。
...
2. 收集该工作进程拥有的所有文档列表，通过回调通知上层
3. 清除路由表中该工作进程的所有绑定关系
4. 如果未超过最大重启次数，启动新的工作进程
5. 上层将受影响的 Session 标记为脏，在下次请求时触发重新编译
6. 后续对这些文档的请求会自动路由到新的（或其他可用的）工作进程
7. 用户可能感受到一次短暂的延迟（AST 需要重新编译），但不会丢失任何编辑内容——文本缓冲区存在主进程的 Session 中

### 无状态工作进程崩溃

1. 主进程检测到退出
2. 触发崩溃退避，降低并发上限
3. 崩溃工作进程上正在执行的任务收到错误；上层调度（Compiler/Indexer）通过自身的队列处理重试
4. 如果未超过最大重启次数，启动新的工作进程
