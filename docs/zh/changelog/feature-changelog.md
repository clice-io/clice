...
- **多进程架构。** 编译在主进程协调下的工作子进程中执行，单个翻译单元的编译器崩溃不再拖垮整个会话——崩溃的工作进程会被重新拉起，其负责的文档按需恢复。工作进程有内存上限，有状态工作进程以 LRU 淘汰驻留文档。
...
- **统一磁盘缓存。** PCH、PCM 与索引产物存放在同一个内容寻址存储中，写入崩溃安全、按大小淘汰，跨会话与重启复用。
...
- **工具链基线。** clice 现基于 LLVM/Clang 21.1.8 构建。
...
- 保存头文件现在会正确地重索引包含它的已关闭文件，保持跨文件引用新鲜。
