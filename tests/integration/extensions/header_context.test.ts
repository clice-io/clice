...
import { writeEntries } from "../../tools/compile_commands.ts";
...
test("query context returns host sources", async ({ session }) => {
    const { client, workspace } = await session("header_context");
    await client.openAndWait(path.join(workspace, "main.cpp"));

    const [utilsUri] = client.open(path.join(workspace, "utils.h"));

    const result = (await client.queryContext(utilsUri)) as QueryContextResult;
...
test("query context source file returns cdb entries", async ({ session }) => {
    ...
    const result = (await client.queryContext(mainUri)) as QueryContextResult;
    expect(result).not.toBeNull();
...
test("current context default null", async ({ session }) => {
    ...
    const result = (await client.currentContext(utilsUri)) as CurrentContextResult;
    expect(result).not.toBeNull();
...
test("switch context and current context", async ({ session }) => {
    ...
    const switchResult = (await client.switchContext(utilsUri, mainUri)) as SwitchContextResult;
    expect(switchResult).not.toBeNull();
    expect(switchResult.success).toBe(true);

    // Verify currentContext now returns main.cpp.
    const current = (await client.currentContext(utilsUri)) as CurrentContextResult;
...
test("full context flow", async ({ session }) => {
    ...
    const query = (await client.queryContext(utilsUri)) as QueryContextResult;
    ...
    const switched = (await client.switchContext(utilsUri, mainUri)) as SwitchContextResult;
    expect(switched.success).toBe(true);

    // 6. currentContext on utils.h -> should now be main.cpp.
    const current2 = (await client.currentContext(utilsUri)) as CurrentContextResult;
...
test("deep nested header context", async ({ session }) => {
    ...
    const [innerUri] = client.open(path.join(workspace, "inner.h"));

    // queryContext on inner.h should find main.cpp through the chain.
    const result = (await client.queryContext(innerUri)) as QueryContextResult;
    expect(result).not.toBeNull();
...
test("deep nested switch context and hover", async ({ session }) => {
    ...
    const switched = (await client.switchContext(innerUri, mainUri)) as SwitchContextResult;
    expect(switched.success).toBe(true);
...
test("query context multiple cdb entries", async ({ session }) => {
    const { client, workspace } = await session("multi_context");
    const [mainUri] = await client.openAndWait(path.join(workspace, "main.cpp"));
...
test("switch between two hosts", async ({ session }) => {
    ...
    const [aUri] = await client.openAndWait(path.join(workspace, "a.cpp"));
    const [bUri] = await client.openAndWait(path.join(workspace, "b.cpp"));
    ...
    let switched = (await client.switchContext(sharedUri, aUri)) as SwitchContextResult;
    expect(switched.success).toBe(true);
    ...
    switched = (await client.switchContext(sharedUri, bUri)) as SwitchContextResult;
    expect(switched.success).toBe(true);
