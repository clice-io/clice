...
export class CliceClient {
    ...
    private constructor(child: ChildProcessWithoutNullStreams, transport: Transport) {
        ...
        this.connection.onNotification(proto.PublishDiagnosticsNotification.type, (params) => {
    ...
    async initialize(
        ...
        const result = await this.connection.sendRequest(proto.InitializeRequest.type, params);
        await this.connection.sendNotification(proto.InitializedNotification.type, {});
    ...
    open(filepath: string, version = 0, options: { text?: string } = {}): [string, string] {
        ...
        void this.connection.sendNotification(proto.DidOpenTextDocumentNotification.type, {
            textDocument: {
                uri: wireUri,
                languageId: "cpp",
                version,
    ...
    close(uri: string): void {
        void this.connection.sendNotification(proto.DidCloseTextDocumentNotification.type, {
            textDocument: { uri },
        });
    }
    ...
    change(uri: string, version: number, text: string): void {
        void this.connection.sendNotification(proto.DidChangeTextDocumentNotification.type, {
    ...
    save(uri: string, text?: string): void {
        void this.connection.sendNotification(proto.DidSaveTextDocumentNotification.type, {
            textDocument: { uri },
            ...(text === undefined ? {} : { text }),
        });
    }
    ...
    formatRange(uri: string, range: proto.Range) {
        return this.connection.sendRequest(proto.DocumentRangeFormattingRequest.type, {
            textDocument: { uri },
            range,
            options: { tabSize: 4, insertSpaces: true },
        });
    ...
    callHierarchyIncoming(item: proto.CallHierarchyItem) {
        return this.connection.sendRequest(proto.CallHierarchyIncomingCallsRequest.type, { item });
    }

    callHierarchyOutgoing(item: proto.CallHierarchyItem) {
        return this.connection.sendRequest(proto.CallHierarchyOutgoingCallsRequest.type, { item });
    }
    ...
    typeHierarchySupertypes(item: proto.TypeHierarchyItem) {
        return this.connection.sendRequest(proto.TypeHierarchySupertypesRequest.type, { item });
    }

    typeHierarchySubtypes(item: proto.TypeHierarchyItem) {
        return this.connection.sendRequest(proto.TypeHierarchySubtypesRequest.type, { item });
    }
