...
export async function waitForIndex(
  ...
  for (let i = 0; i < timeoutSeconds; i++) {
    ...
    if (result && result.some((s) => s.name === symbolName)) {
      return true;
    }
    await sleep(1_000);
  }
...
export function anomaliesInLogMessages(client: CliceClient): string[] {
  ...
  for (const msg of client.logMessages) {
    const match = ANOMALY_PATTERN.exec(msg.message);
    if (match) {
      found.push(match[1]!);
    }
  }
...
export function assertNoAnomaly(
  client: CliceClient,
  workspace: string | null = null,
): void {
  const found = [
    ...anomaliesInLogMessages(client),
    ...anomaliesInLogFiles(workspace),
  ];
  ...
  if (found.length > 0) {
    throw new Error(`clice reported internal anomalies: ${found}${detail}`);
  }
}
