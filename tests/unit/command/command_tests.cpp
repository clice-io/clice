...
TEST_CASE(Reuse) {
    ...
    auto cmd1 = database.lookup("test.cpp", options).front();
    auto cmd2 = database.lookup("test2.cpp", options).front();
    // flags: [clang++, -std=c++23] (no source file)
    ASSERT_EQ(cmd1.resolved.flags.size(), 2U);
    ASSERT_EQ(cmd2.resolved.flags.size(), 2U);

    ASSERT_EQ(cmd1.resolved.flags[0], "clang++"sv);
    ASSERT_EQ(cmd1.resolved.flags[1], "-std=c++23"sv);
    ASSERT_EQ(cmd1.source_file, "test.cpp"sv);

    // Canonical flags are pointer-shared.
    ASSERT_EQ(cmd1.resolved.flags[0], cmd2.resolved.flags[0]);
    ASSERT_EQ(cmd1.resolved.flags[1], cmd2.resolved.flags[1]);
    ASSERT_EQ(cmd2.source_file, "test2.cpp"sv);
};
...
TEST_CASE(DefaultFallback) {
    ...
    auto& cpp_cmd = cpp_results.front();
    auto cpp_argv = cpp_cmd.to_argv();
    ...
    ASSERT_EQ(h_results.front().resolved.flags[0], "clang"sv);
};
...
TEST_CASE(ResolvePath) {
    ...
    auto cmd = database.lookup("test/main.cpp", quiet_options()).front();
    /// source_file is the resolved path from PathPool.
    ASSERT_EQ(cmd.source_file, "test/main.cpp"sv);
};
...
TEST_CASE(MoveSemantics) {
    ...
    auto options = quiet_options();
    auto cmd = db2.lookup("main.cpp", options).front();
    ASSERT_EQ(cmd.resolved.flags.size(), 2U);
    ASSERT_EQ(cmd.resolved.flags[1], "-std=c++23"sv);

    /// Move assign.
    CompilationDatabase db3;
    db3 = std::move(db2);
    cmd = db3.lookup("main.cpp", options).front();
    ASSERT_EQ(cmd.resolved.flags.size(), 2U);
    ASSERT_EQ(cmd.resolved.flags[1], "-std=c++23"sv);
};
...
TEST_CASE(ResourceDir) {
    ...
    auto cmd_no_tc = database.lookup("main.cpp").front();
    ASSERT_EQ(cmd_no_tc.resolved.flags.size(), 2U);
    ASSERT_EQ(cmd_no_tc.resolved.flags[0], "clang++"sv);
    ASSERT_EQ(cmd_no_tc.resolved.flags[1], "-std=c++23"sv);
    ASSERT_EQ(cmd_no_tc.source_file, "main.cpp"sv);

    // With query_toolchain, resource dir is present in the result.
    auto cmd_tc = database.lookup("main.cpp", {.query_toolchain = true}).front();
    auto args_tc = cmd_tc.resolved.flags;
