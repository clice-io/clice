...
std::optional<protocol::DocumentHighlight> to_highlight(CompilationUnitRef unit,
                                                        clang::SourceLocation location,
                                                        const PositionMapper& mapper) {
    auto highlight = to_highlight(unit, location);
    if(!highlight)
        return std::nullopt;
...
auto highlight_locations(CompilationUnitRef unit, std::uint32_t offset)
...
auto document_highlights(CompilationUnitRef unit, std::uint32_t offset)
    ...
    for(auto location: highlight_locations(unit, offset)) {
...
auto document_highlights(CompilationUnitRef unit, std::uint32_t offset, PositionEncoding encoding)
    ...
    for(auto location: highlight_locations(unit, offset)) {
        if(auto highlight = to_highlight(unit, location, mapper)) {
            result.push_back(*highlight);
        }
    }
    return result;
